...
class StageRepositoryImpl implements StageRepository {
  ...
  Future<List<Stage>> getAllStages({String mode = 'clue'}) async {
    ...
    final stagesRaw =
        await db.query('stages', orderBy: 'order_index ASC');
    ...
    for (final raw in stagesRaw) {
      ...
      final movieIds =
          (jsonDecode(raw['film_ids'] as String) as List).cast<int>();
      final completedCount = movieIds
          .where((mid) => completedKeys.contains('${id}_$mid'))
          .length;
      final allDone =
          movieIds.isNotEmpty && completedCount == movieIds.length;
  ...
  @override
  Future<void> markMovieCompleted(int stageId, int movieId, {String mode = 'clue'}) async {
    final db = await _db.database;
    await db.insert(
      'stage_progress',
      {'stage_id': stageId, 'movie_id': movieId, 'mode': mode, 'status': 'completed'},
